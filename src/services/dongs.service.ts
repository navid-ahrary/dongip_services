...
export class DongsService {
  ...
  public async createNewDong (
    ...
    for (const _b of bill) {
      ...
      const relList = await this.usersRepository.usersRels(currentUserId)
        .find({where: {_id: _b.belongsToUserRelsId}});

      if (relList.length === 0) {
        debug('You have not relation with some of users!' + _b.belongsToUserRelsId);
    ...
    exManUsersRel = await this.usersRepository.usersRels(currentUserId)
    ...
    findedCategoryList = await this.usersRepository.categories(currentUserId)
    ...
    transaction = await this.usersRepository
      .createHumanKindDongs(exManId, newDong);
    ...
    for (const _b of bill) {
      ...
      switch (_b.userId.split('/')[0]) {
        case 'Users':
          userRel = await this.usersRepository.usersRels(_b.userId)
          ...
          user = await this.usersRepository
            .findById(_b.userId.split('/')[1]);

          notifyPayload = {
            notification: {
              title: 'دنگیپ دنگ جدید',
              body: nodeCategory.title +
                'توسط' +
                userRel[0].alias +
                'دنگیپ شد'
            },
          ...
          this.firebaseService.sendToDeviceMessage(
            user.registerationToken, notifyPayload, notifyOptions
          );
          break;

        case 'VirtualUsers':
          userRel = await this.virtualUsersRepository.usersRels(_b.userId)
            .find({where: {_to: exManId}});
          nodeCategoryBill['belongsToUserRelsId'] = userRel[0]._id;
