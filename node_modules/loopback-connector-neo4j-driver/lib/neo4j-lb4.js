...
const debug = require('debug')('loopback:connector:neo4j-driver');
 ...
 *
 * @param {Object} options - the data source settings
 * @returns {string}
 */
function generateNeo4jUrl(options) {
...
exports.generateAuthStrategy = generateAuthStrategy;

/**
 * Generate the neo4h authentication strategy
 *
 * @param {Object} options - the data source setting
 */
function generateAuthStrategy(options) {
    options.username = options.user || options.username || 'neo4j';
    options.password = options.pass || options.password || 'neo4j';
    options.authScheme = options.authScheme || 'basic';

    if (options.debug) {
        debug(`Authentication scheme: ${options.authScheme}`);
    }

    switch (options.authScheme) {
        case 'basic':
            return neo4j.auth.basic(options.username, options.password);
        case 'kerberos':
            if (!options.ticket) {
                if (options.debug) {
                    debug('kerberos strategy needs a \'base64EncodedTicket\' ticket');
                }
                return;
            }
            return neo4j.auth.kerberos(options.ticket);
    }
}
...
exports.initialize = function initialize(dataSource, callback) {
    ...
    dataSource.connector = new Neo4j(dataSource.settings, dataSource);
...
function Neo4j(settings, dataSource) {
    Connector.call(this, 'neo4j', settings);
...
Neo4j.prototype.connect = function (callback) {
    ...
    const url = generateNeo4jUrl(self.settings),
        authStrategy = generateAuthStrategy(self.settings);

    this.driver = neo4j.driver(url, authStrategy, validConfigs);
    ...
    session.writeTransaction(tx => {
        tx.run(query, params)
            .then(result => {
                ...
                if (result) {
                    if (self.debug) {
                        debug('create: user,session: closed');
                    }
                    if (callback) {
                        callback(null);
                    }
    ...
    session.readTransaction( tx => {
        tx.run(query,params)
            .then(result => {
                ...
                if (response) {
                    data = result.map(function (entry) {
