...
const debug = require("debug")("loopback:connector:neo4j-graph");
const Promise = require("bluebird");
const uuid = require('uuid');


exports.generateNeo4jUrl = generateNeo4jUrl;
/**
 ...
 * @returns {string}
 */
function generateNeo4jUrl(settings) {
    "use strict";
    settings.hostname = settings.hostname || settings.host || "localhost";
    settings.port = settings.port || 7687;

    return "bolt://" + settings.hostname + ":" + settings.port;
};

exports.Neo4jAuthStrategy = Neo4jAuthStrategy;

/**
 * Generate No4 authentication strategy
 */
function Neo4jAuthStrategy(settings) {
    "user strict";

    const username = settings.user || settings.username || 'neo4j';
    const password = settings.pass || settings.password || 'neo4j';
    var authStrategy = settings.authStrategy || 'basic';

    if (settings.debug) {
        debug(`Authentication strategy: ${authStrategy}`);
    }
    ...
    switch (authStrategy) {
        ...
        case 'kerberos':
            if (settings.ticket) {
                var ticket = settings.ticket;
                return neo4j.auth.kerberos(ticket);
            } else {
                console.error('base64EncodedTicket for kerberos strategy must be provided');
                return;
            }
    }
 ...
 * @constructor
 */
function Neo4j(settings, dataSource) {
    "use strict";
    Connector.call(this, "neo4j-driver", settings);
 ...
 * @param {Function} callback the callback function
 */
Neo4j.prototype.connect = function (callback) {
    var self = this,
        validOptionName = [],
        validConfigNames = [
            'encrypted',
    ...
    if (self.settings.driverConfigs) {
        var driverConfigsName = Object.keys(self.settings.driverConfigs);
        driverConfigsName.forEach(function (conf) {
            if (validConfigNames.indexOf(driverConfigs) > -1) {
                validConfigs[conf] = driver(self.settings.driverConfigs[conf]);
            }
        });

        if (self.debug) debug(`Valid Configs: ${validConfigs}`);
    }

    const url = self.settings.url || generateNeo4jUrl(self.settings),
        strategy = Neo4jAuthStrategy(self.settings);
    ...
    if (self.debug) {
        debug(`Neo4j connection is established: ${url}`)
    }

    if (callback) {
        callback(null, self.driver);
    }
}
...
Neo4j.prototype.create = function (model, data, options, callback) {
    "use strict";
    var self = this,
        session = self.driver.session(neo4j.session.WRITE),
    ...
    session.run(`CREATE (n: ${label} $props)`, {
            "props": data
        })
        .then(() => {
            session.close(() => {
                if (self.debug) {
                    debug(`create:${data.name} is created,sessoon:is closed`);
                }
                if (callback) {
                    callback(null);
                }
...
Neo4j.prototype.save = function (model, data, options, callback) {
    ...
    session.run(
        "MERGE (n:" + self.label(model) + " {" + idName + ": {id}}) ON CREATE SET n = {properties} ON MATCH SET n = {properties} RETURN n",
        params
...
Neo4j.prototype.exists = function (model, id, options, callback) {
    ...
    session.run(
        "MATCH (n:" + self.label(model) + " {" + idName + ": {id}}) RETURN n.{idName}",
        params
    ).then(function (error, response) {
        session.close();
...
Neo4j.prototype.find = function (model, id, options, callback) {
    ...
    session.run(
        "MATCH (n:" + self.label(model) + " {" + idName + ": {id}}) RETURN n",
        params
    ).then(function (error, response) {
        session.close();
...
Neo4j.prototype.destroy = function (model, id, options, callback) {
    ...
    session.run(
        "MATCH (n:" + self.label(model) + " {" + idName + ": {id}}) DETACH DELETE n",
        params
    ).then(function (error, response) {
        session.close();
...
Neo4j.prototype.all = function (model, filter, options, callback) {
    "use strict";
    var self = this,
        session = self.driver.session(neo4j.session.READ),
        query = `MATCH (n: ${self.label(model)} )`,
        params = {},
        where = {},
        order = "",
        ...
        .then(response => {
            ...
            if (response) {
                ...
                response.records.forEach(function (record) {
                    var properties = self.fromDatabase(model, record._fields[0]['properties']);
...
Neo4j.prototype.destroyAll = function (model, where, options, callback) {
    ...
    session.run(query, params)
        .then(function (error, response) {
            ...
            if (callback) {
                callback(error, response ? response[0] : undefined);
            }
        });
};
...
Neo4j.prototype.count = function (model, where, options, callback) {
    ...
    session.run(query, params)
        .then(response => {
            ...
            session.close(() => {
                if (self.debug) {
                    debug("count: response:%j", response);
                }
...
Neo4j.prototype.updateAttributes = function (model, id, data, options, callback) {
    ...
    session.run(
        "MATCH (n:" + self.label(model) + " {" + idName + ": {id}}) SET n += {properties} RETURN n",
        params
    ).then(function (error, response) {
        if (self.debug) {
 ...
 * @param {Function} callback - The callback function
 */
Neo4j.prototype.replaceById = function (model, id, data, options, callback) {
    "use strict";
    var self = this,
        session = self.driver.session(),
        idName = self.getIdName(model),
    ...
    session.run(
        ...
        if (self.debug) {
            debug("replaceById:error:%j,response:%j", error, response);
        }
        if (callback) {
            callback(error, response);
        }
...
Neo4j.prototype.updateAll = function (model, where, data, options, callback) {
    ...
    session.run(query, params)
        .then(response => {
            if (self.debug) {
                debug("updateAll:response:%j", response);
            }
...
Neo4j.prototype.execute = function (command, params, options, callback) {
    ...
    return session.run(cypher.query, options).then(function () {
        session.close();
    });
};
    ...
    models.forEach(function (model) {
        ...
        if (uniquePropertiesArray.length) {
            uniquePropertiesArray.forEach(function (property) {
                queries.push(
                    "CREATE CONSTRAINT ON (n:" + self.label(model) + ") ASSERT n." + property + " IS UNIQUE"
                );
            });
        }
        ...
        if (indexPropertiesArray.length) {
            indexPropertiesArray.forEach(function (property) {
                queries.push(
                    "CREATE INDEX ON :" + self.label(model) + "(" + property + ")"
                );
            });
        }
        ...
        if (self.settings.enterprise) {
            ...
            if (existPropertiesArray.length) {
                existPropertiesArray.forEach(function (property) {
                    queries.push(
                        "CREATE CONSTRAINT ON (n:" + self.label(model) + ") ASSERT EXISTS(n." + property + ")"
                    );
                });
            }
        }
...
Neo4j.prototype.automigrate = function (models, callback) {
    ...
    var self = this,
        labels = {},
        neo4jHttp = Promise.promisify(self.driver.http, {
            "context": self.driver
        }),
        neo4jCypher = Promise.promisify(self.driver.cypher, {
            "context": self.driver
        });
