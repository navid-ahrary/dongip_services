...
function Neo4jAuthStrategy(settings) {
    ...
    const authScheme = settings.authScheme || 'basic';

    if (settings.debug) {
        debug(`Authentication scheme: ${authScheme}`);
    }
...
Neo4j.prototype.create = function (model, data, options, callback) {
    ...
    query = `CREATE (n: ${label} $properties)`;
    ...
    session.writeTransaction(tx => {
        tx.run(query, params)
            .then(result => {
                ...
                if (result) {
                    if (self.debug) {
                        debug(`create:${idName} is created,sessoon:is closed`);
                    }
                    if (callback) {
                        callback(null);
                    }
...
Neo4j.prototype.exists = function (model, id, options, callback) {
    ...
    session.run(
        ...
        .then(response => {
            ...
            if (self.debug) {
                debug("exists:response:%j", response);
            }
            if (callback) {
                callback(error, !!response);
            }
    ...
    session.readTransaction(tx => {
        tx.run(query, params)
            .then(response => {
                ...
                session.close(() => {
                    if (self.debug) {
                        debug('count.callback', model, count);
                    }
        ...
        tx.run(query, params).then(result => {
            ...
            if (result) {
                ...
                result.records.forEach(record => {
                    var count = record.get('count').toNumber();
                    console.log(count);
                    if (callback) {
